...
public class BresenhamFull : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < lines.Count; i++)
        {
            ...
            if (Math.Abs(p2.y - p1.y) < Math.Abs(p2.x - p1.x))
            {
                if (p1.x > p2.x)
                {
                    LineDrawing.Bresenham_Low((int)p2.x, (int)p2.y, (int)p1.x, (int)p1.y, lineDrawnTexture);
                }
                else
                {
                    LineDrawing.Bresenham_Low((int)p1.x, (int)p1.y, (int)p2.x, (int)p2.y, lineDrawnTexture);
                }
            }
            else
            {
                if (p1.y > p2.y)
                {
                    LineDrawing.Bresenham_High((int)p2.x, (int)p2.y, (int)p1.x, (int)p1.y, lineDrawnTexture);
                }
                else
                {

                    LineDrawing.Bresenham_High((int)p1.x, (int)p1.y, (int)p2.x, (int)p2.y, lineDrawnTexture);
                }
            }
        }
